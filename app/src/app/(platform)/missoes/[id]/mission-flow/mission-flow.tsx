...
import { chunkIntoParagraphs, parseMissionContent } from "./parse-mission-content";
import { useStudentWork } from "./use-student-work";

const TOTAL_STEPS = 9;

/**
 * Mission Flow — a Missão como sequência de microetapas, não como página
 * única (docs/MISSION_STUDIO.md continua sendo a autoria; isto é a
 * experiência de quem investiga). Ver decisões em DECISIONS.md D-027.
 */
export function MissionFlow({ mission }: { mission: Mission }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
      ...
      {step === 1 && (
        <CoverStep mission={mission} onStart={() => goTo(2)} />
      )}
      ...
      {step === 7 && (
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            {parsed.auditCriteria.map((entry) => (
              <RubricCard key={entry.label} entry={entry} />
            ))}
          </div>
      ...
      {/* Competências — nota de rodapé discreta, fora do fluxo de etapas */}
...
function CoverStep({
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-6 text-center">
      <span className="flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/25 to-chart-2/20 text-primary">
...
function InvestigationStep({
  ...
  if (isIntro) {
    return (
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          {parsed.investigationGuide.map((entry) => (
            <RubricCard key={entry.label} entry={entry} />
          ))}
        </div>
        <MissionNavigation
