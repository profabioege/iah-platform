...
## D-046 — Divisão do papel "admin" em Mantenedor / Direção / Coordenação Pedagógica (21/07/2026)

**Decisão:** O papel institucional único `admin` (rótulo "Administrador(a) Institucional", servido por `/gestor`) é dividido em três papéis reais: `maintainer` (Mantenedor, rede/grupo de unidades), `director` (Direção, uma unidade escolar) e `pedagogical_coordinator` (Coordenação Pedagógica, pedagogia da unidade) — nunca mais "Gestor" genérico na interface. `teacher` e `student` não mudam. Cada papel novo ganha rota própria (`/mantenedor`, `/direcao`, `/coordenacao`), menu lateral enxuto (itens exatos definidos pelo produto), Início com no máximo 4 indicadores + 2 blocos de atenção (sem sobrecarga cognitiva) e um hub de relatórios navegável (`/[papel]/relatorios/[relatorio]`, mesma arquitetura de `docente-iah/tarefa/[slug]`, ainda sem geração de PDF — não há biblioteca nenhuma no projeto para isso). `/gestor` (`page.tsx`, `executive-dashboard.tsx`) é removido; `gestor/implantacao/*` é movido para `direcao/implantacao/` (deixa de ser item de menu de topo, vira link a partir do Início de Direção); `gestor/assessment-indicators.tsx` é movido para `coordenacao/` (o "desempenho por questão" é escopo pedagógico, não operacional).

`profiles.role` no Supabase é `text check (role in ('aluno','professor','administrador','admin_iah'))` (migration `20260720000300_identity.sql`) — um novo valor de papel exige migration antes de existir de verdade no banco. Por isso: (1) a migration `20260720000800_split_admin_role.sql` foi **criada, aditiva, e não aplicada** nesta etapa; (2) `db/seed/seed-demo.mjs` ganhou dois `insert` novos (mantenedor, coordenador_pedagogico), também não executados; (3) `toWorkspaceRole()` (`modules/workspace/infrastructure/session.ts`) mapeia `"administrador"`/`"admin_iah"` → `director` — a única conta real existente (Fabiana Ege) passa a ver o Painel de Direção imediatamente, sem nenhuma escrita no banco, decisão confirmada com o responsável pelo produto antes de implementar.

**Motivo:** Pedido de produto explícito — um papel genérico misturava três públicos com escopos, linguagem e necessidade de dado muito diferentes (rede, unidade, pedagogia) num painel só de 6 abas.

**Alternativas descartadas:** manter `admin` como valor de banco e diferenciar os três só na interface (rejeitado — o pedido exige validação real no servidor, não só separação visual); aplicar a migration e rodar o seed nesta etapa (rejeitado — instrução explícita de não alterar Supabase/seed sem autorização; ambos ficam prontos, não executados); mapear `"administrador"` para `maintainer` em vez de `director` (rejeitado — Direção é o sucessor mais próximo do que esse papel sempre fez, gestão operacional de uma unidade).

**Impacto futuro:** Mantenedor e Coordenação Pedagógica ficam prontos em código (rotas, menus, páginas, gate de autorização em `middleware.ts`/`auth.config.ts`) mas só são logáveis de verdade depois que `20260720000800_split_admin_role.sql` for aplicada e `seed-demo.mjs` for executado — nenhum dos dois nesta etapa. Um modelo real de "rede"/organização multi-instituição (tabela nova, FK em `institutions`) segue deferido; o Início do Mantenedor trata `institutions.list()` como a rede, e com 1 instituição só mostra estado vazio honesto na comparação entre unidades.
