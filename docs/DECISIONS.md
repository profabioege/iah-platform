...
## D-047 — Processamento assíncrono orientado a eventos e integração do Hermes Agent (28/07/2026)

**Status:** Aprovado pelo Founder. Nenhuma linha de código, migration, endpoint ou workflow foi criada por esta decisão — o registro precede a implementação, conforme `CLAUDE.md`. Detalhamento técnico completo (modelo de dados, contratos JSON, diagramas, plano de 14 micro missões) em `docs/architecture/iah-event-driven-hermes-agent.md`.

**Contexto e problema:** Operações de IA do IAH podem levar de dezenas de segundos a minutos — composição completa de missão, geração de material, síntese pedagógica de tentativa, sugestão de nota, análises agregadas do MaestrIAH, processamento documental. O desenho atual é inteiramente síncrono: uma Server Action mantém a requisição HTTP aberta por até 25 s (`llm-provider-factory.ts`, timeout por capability) e o resultado fica acoplado à sessão do navegador. Três limites tornam isso inviável para tarefas longas, e nenhum é hipotético: (1) o ambiente serverless encerra a função ao fim da resposta HTTP, sem onde uma tarefa de minutos continuar; (2) o estado em memória não sobrevive nem escala — `circuit-breaker.ts` (`const circuits = new Map`) e `preview-limits.ts` (`const inFlightUsers = new Set`) são por processo, e ambos já documentam essa limitação honestamente; (3) fechar a aba durante a operação perde o trabalho sem registro. A auditoria que precedeu esta decisão confirmou que hoje não existe nenhuma tabela de job, fila, worker, webhook ou uso de Supabase Realtime no repositório, e que apenas um arquivo (`professor/docente-iah/apresentacao-slides/actions.ts`) chama o IAH AI Gateway.

**Decisão:**

**1. Fila do MVP — PostgreSQL/Supabase é a fila durável e a fonte de verdade dos jobs.** A reivindicação de trabalho é atômica, por mecanismo equivalente a `SELECT ... FOR UPDATE SKIP LOCKED`. O desenho exige: `idempotency_key` (deduplicação por tenant), `correlation_id` (rastreio ponta a ponta), status versionado, lock com expiração, heartbeat, recuperação de jobs abandonados, retries com backoff exponencial **e jitter**, dead-letter e auditoria append-only das transições. O jitter não é detalhe: sem ele, N jobs que falharam juntos por queda de provedor voltam juntos e derrubam o provedor de novo.

**2. Arquitetura-alvo — PostgreSQL permanece a fonte de verdade.** Inngest ou Trigger.dev poderá futuramente assumir execução distribuída, agendamento, cron, retries e observabilidade operacional. Essa evolução **não pode** transformar o serviço gerenciado na única fonte de verdade dos jobs; a tabela continua sendo o registro durável e auditável.

**3. Hermes Agent — worker externo, sem acesso direto ao banco.** O Hermes não acessa o Supabase diretamente, não recebe `service_role`, não recebe connection string, não executa SQL arbitrário, não é fonte de verdade do estado do job e não consulta dados fora da instituição e do job autorizados. Toda comunicação passa por uma API interna autenticada, de escopo mínimo, com cinco operações conceituais: `claim`, `heartbeat`, `complete`, `fail` e leitura do estado de cancelamento. A razão é concreta: a service role ignora RLS por desenho do Postgres — entregá-la a um worker externo anularia de uma vez todo o isolamento multi-tenant construído em D-023 e D-041.

**4. Notificação do MVP — polling inteligente com backoff (2 s → 5 s → 15 s).** Pausa quando a aba não está visível, retoma ao voltar ao primeiro plano, para em estados terminais, nunca faz polling infinito, recupera o estado após recarregamento ou nova sessão, e limita consumo por usuário. **Supabase Realtime fica registrado como evolução futura**, não como recusa permanente: a auditoria mediu 41 tabelas com RLS habilitada e **zero políticas** — o Realtime respeita a RLS do papel `authenticated`, então hoje um cliente conectaria e não receberia evento algum. Habilitá-lo exige criar políticas RLS específicas e seguras para os jobs, decisão de segurança que não se toma dentro de uma tarefa de fila.

**5. Primeiro fluxo assíncrono — `docentiah.compose_mission`.** Classificação oficial das capabilities: criação completa de missão, geração de slides e materiais, síntese extensa de tentativa e relatórios complexos do MaestrIAH são **assíncronos**; pequenas reescritas são **híbridas**; a **conversa socrática cotidiana do MentorIAH permanece síncrona** — a latência faz parte da experiência pedagógica, e a persistência desse fluxo já foi validada ponta a ponta contra banco real.

**6. n8n, Make.com e ferramentas No-Code** podem atuar como adaptadores, integrações externas, notificações e automações administrativas. Não podem ser banco principal, fila transacional principal, fonte de verdade, sistema de autorização nem mecanismo principal de isolamento institucional.

**7. Privilégios SQL declarados explicitamente.** Toda migration nova declara os privilégios que precisa, sem depender exclusivamente de `ALTER DEFAULT PRIVILEGES`. Para as tabelas internas de jobs: `service_role` recebe somente o necessário, `anon` não recebe acesso, `authenticated` não recebe acesso direto enquanto o consumo ocorrer pela API, RLS permanece habilitada e nenhuma política permissiva genérica é criada. Motivo verificado em campo: as tabelas criadas após a migration `20260720000700_service_role_grants.sql` não têm GRANT próprio e dependem do `ALTER DEFAULT PRIVILEGES FOR ROLE postgres` — funcionou na aplicação real das 12 migrations, mas é dependência frágil e não declarada, que só se manifestaria em runtime como `permission denied`.

**8. Retenção inicial.** Payload operacional detalhado: 30 dias. Tentativas, métricas e erros sanitizados: 90 dias. Resultado pedagógico definitivo: movido ou referenciado por tabelas do respectivo produto, não retido na fila. Prompts completos e respostas intermediárias: não guardar sem necessidade. Dados pessoais: referência por ID e minimização. Exclusão respeita LGPD e auditoria institucional.

**9. CI obrigatória como Etapa Zero.** Antes de qualquer implementação da fila: lint, TypeScript, build, testes, verificação de migrations e bloqueio da integração em caso de falha. O deploy contínuo atual **não é suficiente** — `docs/DEPLOY.md` documenta push na `main` → deploy automático na Vercel, ou seja, existe CD sem CI: os 210 testes só rodam se alguém lembrar. A branch principal não deve chegar à Vercel sem checks obrigatórios aprovados.

**10. Ordem de implementação.** Etapa 0 (CI obrigatória), depois: 1 ADR e contratos · 2 contrato versionado de eventos · 3 máquina de estados · 4 migration de jobs · 5 repositório transacional · 6 API produtora · 7 worker determinístico · 8 API interna do Hermes · 9 integração real do Hermes · 10 retry, heartbeat e dead-letter · 11 polling da interface · 12 `docentiah.compose_mission` · 13 observabilidade e custos · 14 hardening e expansão.

**Motivo:** O projeto tem hoje duas capabilities de IA reais e um único arquivo chamando o Gateway. Introduzir Redis, RabbitMQ ou um serviço gerenciado de jobs adicionaria dependência operacional permanente para um volume que o Postgres resolve com uma tabela e `FOR UPDATE SKIP LOCKED`. O isolamento por `institution_id` já é disciplina consolidada (42 filtros `eq("institution_id")` medidos nos repositórios de banco) — a fila herda isso sem custo. Escolher o Postgres preserva a propriedade mais valiosa do estágio atual: nenhuma peça nova de infraestrutura para operar, monitorar e pagar antes de o produto validar demanda.

**Alternativas descartadas:** *pgmq* (boa, mas adiciona extensão e vocabulário próprio sem ganho no volume atual); *Trigger.dev/Inngest/QStash agora* (dashboard e retries nativos são reais, mas trazem dependência externa e um segundo lugar onde o estado do job existe — fica como arquitetura-alvo, não como MVP); *Redis/BullMQ ou RabbitMQ* (excelentes em concorrência, custo operacional incompatível com o estágio); *n8n/Make como fila* (rejeitado por decisão do Founder e por mérito — não são fonte de verdade transacional, não aplicam autorização nem isolamento institucional); *memória do processo* (já provado inviável: não sobrevive ao encerramento da função serverless); *Supabase Realtime no MVP* (bloqueado por restrição medida do banco — RLS ativa sem políticas silencia o canal); *webhook de retorno do Hermes* (rejeitado em favor da API interna: o webhook cria superfície pública nova e exige assinatura HMAC, anti-replay e rotação de segredo para resolver um problema que a API interna não cria, já que o Hermes autentica para reivindicar).

**Impacto futuro:** Nada do que existe precisa ser reescrito — o IAH AI Gateway, os repositórios e o contrato `LlmProvider` permanecem intactos; a fila os envolve, não os substitui. A partir desta decisão, toda capability nova nasce classificada como síncrona, assíncrona ou híbrida antes de ser implementada. Três consequências colaterais positivas já identificadas: o limite de consumo passa de memória para banco, corrigindo o TOCTOU existente em `apresentacao-slides/actions.ts` (leitura assíncrona do uso diário seguida de lock separado, janela na qual duas requisições concorrentes passam ambas); o fallback silencioso do circuit breaker vira métrica observável (`iah_jobs.provider = 'demo'` acima de um limiar = alerta), hoje invisível por desenho; e o custo por instituição passa a existir, já que `generation_usage` só é gravada por uma capability. Pendência que a expansão ao MentorIAH vai encontrar: o repositório de sessão do Mentor não tem método para marcar `completed` (`MentorSessionRepository` expõe apenas `findActiveByAssignment`, `create` e `touch`), então uma síntese assíncrona disparada "ao encerrar a sessão" ainda não teria gatilho — lacuna de produto a resolver antes da micro missão 14.
