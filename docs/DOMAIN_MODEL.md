...
| `ClassroomIntegration` | ClassroomIntegration *(especialização de Integração — ver seção 2)* | `ClassroomIntegration` |
| `IntegrationProvider` | IntegrationProvider *(contrato-guarda-chuva — ver seção 2)* | `IntegrationProvider` *(já existe como conceito: `modules/integrations`)* |
| — *(pedido nesta Sprint, sem nome em inglês fornecido)* | Indicadores *(nova — ver seção 2)* | `Indicator` / `ClassIndicators` |

---

## 0. Visão geral — Instituição é a raiz

**Toda entidade operacional deste modelo pertence, direta ou transitivamente, a uma `Escola` (`Institution`).** Não existe Turma, Aluno, Professor, Missão-instância, Produção ou Indicador sem uma Escola de origem — é a fronteira de isolamento (tenant, P3) que torna o IAH uma plataforma multiescola, não um sistema de escola única.

Leitura linear simplificada do percurso institucional (a mesma do fluxo completo, seção 4), da raiz ao dado agregado:
...
**Atenção — isto é uma leitura linear didática, não a topologia real do modelo.** Na prática (ver seções 2–3): Professor não "pertence" a um Ano Letivo, pertence à Escola e leciona Turmas *dentro* de um Ano Letivo; uma Missão não pertence a uma Turma, é aplicada a várias Turmas (é template, P2); um Aluno tem várias Produções, não uma. A cadeia acima existe para dar a ordem de leitura do fluxo (seção 4) — os relacionamentos reais, com cardinalidade, estão na seção 1 e no diagrama da seção 3.
...
### Relacionamentos-chave (cardinalidade)

- Uma **Instituição** possui vários **Anos Letivos** (1:N).
- Um **Professor** pertence a uma **Instituição** (N:1) e leciona uma ou mais **Turmas** (N:N, via vínculo de docência).
- Uma **Turma** pertence a um **Ano Letivo** (N:1) e, transitivamente, a uma Instituição.
- Uma **Turma** possui vários **Alunos**, via **Matrícula** (N:N com estado próprio — não é elo simples).
- Uma **Missão** pode ser aplicada em várias **Turmas** — é template (P2); a aplicação numa turma específica é uma **Atividade** (instância).
- Um **Aluno** possui várias **Produções** (1:N) — uma por Atividade/Missão que realizou.
- Uma **Produção** pertence a uma **Missão**/Atividade (N:1) e a um **Aluno** (N:1).
- Um **Aluno** possui várias **Reflexões** no Diário do Auditor (1:N), cada uma referenciando a Missão que a motivou.
- **Indicadores** são sempre derivados (agregação de Progresso/Produção/Reflexão de um conjunto de Alunos/Turma) — nunca uma fonte de verdade própria, nunca editados diretamente.
- Uma **Instituição** possui zero ou mais **Integrações** configuradas (`ClassroomIntegration` é uma especialização de `Integração` — ver seção 2, contexto Integrações Externas); cada Integração usa um **Provedor** (`IntegrationProvider`) específico.
...
| **Indicadores** | Indicadores (agregação derivada, sem persistência própria) | não implementado — base do Painel do Gestor planejado em `ROADMAP.md` |
| **Integrações externas** | `IntegrationProvider` (contrato-guarda-chuva), `AuthProvider`, `ClassroomProvider`, `ClassroomIntegration` (registro de uma integração configurada) | `modules/integrations` (contratos + mock + stub Google, ver D-019) |
...
#### Indicadores — *nova nesta Sprint*
- **Finalidade:** visão agregada e derivada do progresso de uma Turma/Escola — o que um Gestor (Diretor/Mantenedor/Coordenador) precisa ver, ao contrário do detalhe aluno a aluno que o Professor vê. Base conceitual do Painel do Gestor planejado em `ROADMAP.md`.
- **Atributos:** escopo (turma | escola | ano letivo), adesão (% que iniciou), progresso agregado (% que concluiu), competências mais desenvolvidas, destaques curados (trechos de Reflexão/Produção selecionados), calculado em (timestamp).
- **Relacionamentos:** deriva de `Progresso`, `Produção do Aluno` e `Reflexão` de um conjunto de `Aluno`s de uma `Turma`/`Escola`; referencia `Competência`.
- **Regras:** é sempre uma **projeção calculada**, nunca uma fonte de verdade — não tem ciclo de vida próprio (não é criado/editado, é recalculado); não expõe dado por aluno individual (isso é escopo do Progresso/Painel do Professor, não do Painel do Gestor).
...
#### Integração
- **Finalidade:** conexão configurada com um serviço externo (Google Classroom, Microsoft Teams, Google Agenda, Canva, provedor de IA), com credenciais/escopo. Esta entidade é o registro conceitual de uma integração *configurada e ativa* para uma Escola, que a implementação de código ainda não persiste (não há banco).
- **Atributos:** id, tipo/provedor, escopo (escola ou usuário), credenciais/tokens (referência segura), status, última sincronização.
- **Relacionamentos:** pertence a `Escola` ou `Usuário`; habilita `Agenda`, `Material`, `Ferramenta de IA`; especializa-se em `ClassroomIntegration` quando o propósito é importar/sincronizar Turmas e Alunos.
...
#### ClassroomIntegration — *nova nesta Sprint*
- **Finalidade:** especialização de `Integração` para o caso específico de sincronizar Turmas/Alunos com uma sala de aula externa (Google Classroom, Microsoft Teams, Moodle) — é o registro de "esta Escola conectou este provedor para importar estas Turmas".
- **Atributos:** herda de `Integração` (id, escola, status, última sincronização) + turmas externas mapeadas (`ExternalCourse.id` → `Turma.id` interno), data da última importação, modo (sincronização contínua | importação pontual).
- **Relacionamentos:** pertence a `Escola`; usa um `IntegrationProvider` do tipo `ClassroomProvider`; produz/atualiza `Turma`, `Aluno`, `Matrícula` via a camada de Importação (ver `IMPORT_ARCHITECTURE.md`).
- **Regras:** nunca sobrescreve dados editados manualmente sem confirmação explícita (evita a Escola perder correções feitas à mão); toda sincronização é auditável (o que mudou, quando).

#### IntegrationProvider — *nova nesta Sprint*
- **Finalidade:** contrato-guarda-chuva conceitual que nomeia a família de portas de integração externa do domínio — `AuthProvider` (login) e `ClassroomProvider`/`ImportProvider` (dados de turma) são especializações dele. Já reconhecível em código como o próprio módulo `modules/integrations` (D-019): cada subpasta (`auth/`, `classroom/`) é uma família de `IntegrationProvider`.
- **Atributos conceituais:** `id` do provedor (`"mock" | "google" | "microsoft" | "moodle" | "csv" | "manual"`), `isConfigured` (se há credencial real).
- **Relacionamentos:** todo `AuthProvider`, `ClassroomProvider` e `ImportProvider` (ver `IMPORT_ARCHITECTURE.md`) é um `IntegrationProvider`.
- **Regras:** a UI e o domínio pedagógico nunca dependem de um `IntegrationProvider` concreto — apenas do contrato específico que consomem (`AuthProvider` ou `ImportProvider`), mesma regra já aplicada em D-019.
...
└────────┘        └────────┘        └──────┬───────┘
   ...
   └────────┬────────┘            └───────┬────────┘               └─────┬─────┘
    ...
    └─────────────┘              └────────────────┘            └─────────┬──────────┘
                                                                         │ evidencia
   ┌──────────────┐  desenvolve  ┌──────────────┐                        ▼
   │ Competência  │◀─────────────│    Missão    │                 ┌────────────┐   agrega   ┌─────────────┐
   └──────────────┘              └──────────────┘                 │ Progresso  │───────────▶│ Indicadores │
                                                                  └────────────┘            └─────────────┘
COLABORAÇÃO                 ACERVO         ┌────────────────────┐  (do Aluno)                (Painel do Gestor)
┌──────────┐  ┌───────────┐               │ Diário do Auditor  │──▶ Reflexão (entradas) ──┐
│ Projeto  │  │ Hackathon │               └────────────────────┘                          │ agrega em
└────┬─────┘  └─────┬─────┘                                                               ▼
     └─── de Aluno(s), em Turma, gera Produção ───┘                                  Indicadores

TRANSVERSAIS                              INTEGRAÇÕES EXTERNAS (modules/integrations)
┌──────────┐  sincroniza  ┌──────────────────────┐  usa  ┌────────────────────┐  alimenta  ┌──────────────┐
│  Agenda  │◀─────────────│ ClassroomIntegration │◀──────│ IntegrationProvider│───────────▶│ Turma / Aluno│
└──────────┘ (Google etc.)│ (especializa         │       │ (AuthProvider,     │  (mesmo    │  (internos,  │
   ▲ prazos                │  Integração)         │       │  ImportProvider —  │   modelo)  │  via         │
   │                       └──────────────────────┘       │  ver IMPORT_       │            │  Matrícula)  │
┌──────────────┐◀─ eventos de                             │  ARCHITECTURE.md)  │            └──────────────┘
...
O modelo institucional (seções 1–4) é o **único destino** de qualquer dado de Turma/Aluno, não importa a origem — cadastro manual, CSV, Google Classroom, Microsoft Teams ou Moodle. A arquitetura completa da camada de importação (contrato `ImportProvider`, as 5 implementações previstas, fluxo de revisão humana antes de gravar, reconciliação de identidade) está documentada separadamente em **[IMPORT_ARCHITECTURE.md](IMPORT_ARCHITECTURE.md)**, para não duplicar conteúdo entre os dois documentos.

Resumo: nenhuma tela ou lógica de domínio pode depender de saber qual foi a origem de uma Turma/Aluno — todo `ImportProvider` (especialização de `IntegrationProvider`, seção 2) converge para as mesmas entidades `Turma`, `Matrícula` e `Aluno` desta seção 2, através do mesmo tipo de registro `ClassroomIntegration`.
...
- **Reconciliação de identidade entre origens:** quando Google Classroom e cadastro manual descrevem o "mesmo" aluno, qual é a chave de identidade única? (novo, ver `IMPORT_ARCHITECTURE.md`).

---

## 10. Pontos de extensão

Onde este modelo foi desenhado para crescer sem precisar ser redesenhado:

- **Novo papel de Usuário** (ex.: Responsável/pais, Coordenador com escopo próprio): adiciona-se um valor em `Perfil.papel` — não exige nova hierarquia de classes (P1).
- **Novo provedor de importação/integração** (ex.: Moodle, um LMS novo): implementa `ImportProvider` (ou `AuthProvider`, se também autenticar) em `infrastructure/` — ver `IMPORT_ARCHITECTURE.md`. Nenhuma entidade do domínio muda.
- **Nova Missão ou Curso inteiro:** é conteúdo (`Missão`/`Módulo`/`Curso`), não schema — já resolvido hoje via `src/content/missions/*` (D-018/PRODUCT.md); o modelo institucional não precisa mudar para a Missão 02 existir.
- **Novo indicador no Painel do Gestor:** `Indicadores` é uma projeção calculada (seção 2) — um indicador novo é uma nova agregação sobre `Progresso`/`Produção`/`Reflexão` existentes, não uma nova tabela/entidade persistida.
- **Multiescola → múltiplas redes de ensino (franquias):** `Escola` já é o tenant; agrupar várias Escolas sob uma "Rede" é uma entidade adicional acima de Escola, sem alterar nada abaixo dela.
- **Avaliação formal (nota/rubrica):** ver melhoria sugerida 3 (seção 8) — entra como `Rubrica`/`Avaliação` associada a `Produção do Aluno`, sem alterar `Produção` em si.
