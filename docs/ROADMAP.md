...
**Nenhuma em execução.** Última tarefa concluída: M10 — Lesson Architecture (fundação conceitual; nenhuma funcionalidade nova, nenhum código). **Prioridade imediata:** reensaiar o tempo da demonstração — a interface mudou de novo desde `ROTEIRO-DEMONSTRACAO.md` (M08 e M09). Depois: **(a)** fundador executa os passos de console (`AUTHENTICATION.md`/`SUPABASE.md`) → primeiro login real → sincronizar turmas reais do Google Classroom e migrar o Estúdio para banco; **(b)** sem dependência de infra: Painel do Gestor (abaixo) ou a ponte Estúdio → runtime do aluno. A implementação de qualquer componente da `Lesson` (D-028) ainda não foi priorizada — cada componente do Pedagogical Package (Slides, NotebookLM, Avaliação Assistida, Adaptações para Neurodivergentes etc.) deve entrar neste Roadmap como Sprint própria, na ordem que o piloto de agosto exigir.
