...
**Nenhuma em execução.** Última tarefa concluída: M07 — Primeiro Usuário Real (infraestrutura de autenticação definitiva; ativação depende dos passos de console do fundador — `AUTHENTICATION.md`/`SUPABASE.md`). Próxima Sprint natural: **sincronizar as turmas reais do Google Classroom** (implementar o `google-classroom-repository`, hoje stub) assim que o primeiro login real for validado; o Painel do Gestor (abaixo) segue como alternativa sem dependência de infraestrutura.
