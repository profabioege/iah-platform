...
**Nenhuma em execução.** Última tarefa concluída: M07 — Mission Studio (autoria completa neste dispositivo; banco entra com o Supabase). Próximos caminhos: **(a)** fundador executa os passos de console (`AUTHENTICATION.md`/`SUPABASE.md`) → primeiro login real → sincronizar turmas reais do Google Classroom (implementar `google-classroom-repository`, hoje stub) e migrar o Estúdio para banco; **(b)** sem dependência de infra: Painel do Gestor (abaixo) ou a ponte Estúdio → runtime do aluno.
