...
`6fd2612` (anterior a esta tarefa) — *feat(knowledge): M11 — Knowledge Engine* (18/07/2026), branch `main`. Este ciclo (Ciclo 2) segue com o Lesson Builder MVP (M12) — ver "Ciclo 2 — Lesson Builder MVP" abaixo. Ver `CHANGELOG.md` para o histórico completo.
...
- Lesson Builder MVP (`/professor/aulas`, módulo `modules/lesson`, M12): assistente "Nova Lesson" em 6 etapas (Planejamento → Currículo → Mission Flow → Materiais do Knowledge Engine → Preview → Salvar), autosave a cada etapa, Lessons salvas neste dispositivo (localStorage rotulado). Reaproveita `MissionNavigation` (Mission Flow) e o parser de Critérios da Missão selecionada — nenhum componente duplicado.
...
O Lesson Builder MVP (M12) é a primeira tela real do conceito Lesson, mas não muda a prioridade imediata: o Mission Flow mudou de novo (M08 e M09) desde que `ROTEIRO-DEMONSTRACAO.md` foi escrito — **reensaiar o tempo da demonstração segue sendo a tarefa mais urgente**, antes de qualquer outra coisa: a meta de 15 (ou 20?) minutos nunca foi validada com fluxo algum. Em paralelo: **do fundador** (fora do código, ~15 min) — executar os passos de console de `AUTHENTICATION.md`/`SUPABASE.md` (Google Cloud + Supabase) e fazer o primeiro login real; **da próxima Sprint de código**, com o login validado — implementar o `google-classroom-repository` (hoje stub) para sincronizar turmas reais. Painel do Gestor segue planejado no `ROADMAP.md`. Pendências abertas pelo Lesson Builder MVP: nenhuma retomada inteligente de etapa (sempre abre na Etapa 1, mesmo para Lesson já salva); nenhum vínculo com `KnowledgeReference` (a Lesson referencia materiais direto por id, sem registrar o vínculo formal do Knowledge Engine); catálogo de códigos BNCC/BNCC Computação segue sem existir (D-029/D-030) — Competências/Objetivos são entrada livre.
